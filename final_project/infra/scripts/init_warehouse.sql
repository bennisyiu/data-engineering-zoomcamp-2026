...
-- Read-only group. Create login users separately and grant this role; passwords
-- must come from environment variables or a secret manager, never source control.
DO $$
BEGIN
...
GRANT USAGE ON SCHEMA marts TO reviewer;
GRANT SELECT ON ALL TABLES IN SCHEMA marts TO reviewer;
